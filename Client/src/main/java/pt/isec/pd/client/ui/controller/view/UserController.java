...
import pt.isec.pd.client.ui.component.dialog.EditUserDialog;
...
import pt.isec.pd.sharedLib.network.request.User.EditUser;
import pt.isec.pd.sharedLib.network.request.User.GetUser;
	...
	private void editUSerPopUp() {
		try {
			...
			dialog.showAndWait().ifPresent(
					infoUserDTO -> { //TODO: check if can be improved (infoUserDTO → (username, email, phone_number, password))
